...
import { FirstSoundQuestion, FIRST_SOUND_QUESTIONS } from "@/lib/phonemic-data";
import { cn } from "@/lib/utils";

interface FirstSoundGameProps {
  onComplete?: (score: number) => void;
  onExit?: () => void;
}

export function FirstSoundGame({ onComplete, onExit }: FirstSoundGameProps) {
  ...
  const currentQuestion = FIRST_SOUND_QUESTIONS[currentQuestionIndex];
  const isLastQuestion = currentQuestionIndex === FIRST_SOUND_QUESTIONS.length - 1;
  const isCorrect = selectedAnswer === currentQuestion.correctSound;
  ...
  // Play sound option
  const playSoundOption = useCallback((soundOption: string) => {
    const sound = new Howl({
      src: [`/audio/phonemes/${soundOption.replace(/\//g, "")}.mp3`],
      volume: 1.0,
    });
    sound.play();
  }, []);
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (answer === currentQuestion.correctSound) {
  ...
  const handleNextQuestion = () => {
    if (isLastQuestion) {
      ...
      const finalScore = Math.round((totalCorrect / FIRST_SOUND_QUESTIONS.length) * 100);
      setTimeout(() => {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-yellow-100 via-orange-100 to-pink-100 flex items-center justify-center p-4">
        ...
        >
          ...
          <h1 className="text-5xl font-black text-gray-800 mb-4">Excellent Work!</h1>
          <p className="text-2xl text-gray-600 mb-2">
            You identified {totalCorrect} out of {FIRST_SOUND_QUESTIONS.length} first sounds!
          </p>
        </motion.div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto mb-6">
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="text-sm font-bold text-gray-600">
            Question {currentQuestionIndex + 1} of {FIRST_SOUND_QUESTIONS.length}
          </div>
          <button
            onClick={onExit}
        ...
        <div className="w-full h-3 bg-gray-200 rounded-full overflow-hidden">
          <motion.div
            initial={{ width: 0 }}
            animate={{
              width: `${((currentQuestionIndex + 1) / FIRST_SOUND_QUESTIONS.length) * 100}%`,
            }}
                ...
                {currentQuestion.options.map((option, index) => (
                  <motion.button
                    ...
                    onClick={() => {
                      ...
                      setTimeout(() => handleAnswer(option), 500);
                    }}
                    className="py-8 sm:py-10 rounded-2xl bg-gradient-to-br from-blue-400 to-purple-500 text-white font-black text-3xl sm:text-4xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-3"
                  >
              ...
              <div>
                ...
                >
                  ...
                  <p className="text-lg font-bold text-gray-700">
                    The word <span className="text-primary">{currentQuestion.item.word}</span> starts
                    with the <span className="text-primary">{currentQuestion.correctSound}</span> sound.
                  </p>
                </motion.div>

                {/* Show all options with correct/incorrect */}
                <div className="grid grid-cols-2 gap-4 sm:gap-6 max-w-2xl mx-auto mb-6">
                  {currentQuestion.options.map((option) => {
                    const isThisCorrect = option === currentQuestion.correctSound;
                    ...
                    return (
                      <div
                        key={option}
                        className={cn(
                          "py-8 sm:py-10 rounded-2xl font-black text-3xl sm:text-4xl shadow-lg flex items-center justify-center gap-3",
                          isThisCorrect
                      ...
                      >
                        {isThisCorrect && <CheckCircle className="w-8 h-8" />}
                        {wasSelected && !isThisCorrect && <XCircle className="w-8 h-8" />}
                        <span>{option}</span>
                      </div>
                    );
                  })}
